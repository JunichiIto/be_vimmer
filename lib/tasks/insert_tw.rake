...
M               1  光標移動到屏幕的中間一行
N               1  反方向重複最近的 '/' 或 '?' N 次
O               2  在光標上方開始一個新行，並插入內容，重複 N次
["x]P           2  在光標放置內容 [到緩衝區 (見上) x] N 次
Q                  切換到 "Ex" 模式
...
n               1  重複最近的 '/' 或 '?' N 次
o               2  在光標下方開始一個新行，並插入內容，重複 N次
["x]p           2  在光標後放置內容 [從寄存器 x] N 次
...
gJ              2  連接行，不插入空格
["x]gP          2  在光標籤放置內容 [從寄存器 x] N 次，光標留在插入內容之後
gR              2  進入虛擬替換模式
