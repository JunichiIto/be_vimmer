...
<CR>            開始新行
CTRL-M          等同於 <CR>
CTRL-N          查找光標前面的關鍵字的下一個匹配
CTRL-O          執行單個命令然後回到插入模式
CTRL-P          查找光標前面的關鍵字的前一個匹配
...
&               2  重複上次 :s
'{a-zA-Z0-9}    1  光標移動到位置標記 {a-zA-Z0-9} 所在行的第一個 CHAR 上
''              1  光標移動到最近一次跳轉之前所在行的第一個CHAR 上
'(              1  光標移動到當前句子的首部所在行的第一個 CHAR上
')              1  光標移動到當前句子的尾部所在行的第一個 CHAR上
'<              1  光標移動到當前緩衝區高亮區開始/曾經開始的行的第一個 CHAR 上。
'>              1  光標移動到當前緩衝區高亮區結束/曾經結束的行的第一個 CHAR 上。
'[              1  光標移動到當前緩衝區最近操作/放置的文本的首部所在行的第一個 CHAR 上
']              1  光標移動到當前緩衝區最近操作/放置的文本的尾部所在行的第一個 CHAR 上
'{              1  光標移動到當前段落的首部所在行的第一個 CHAR上
'}              1  光標移動到當前段落的尾部所在行的第一個 CHAR上
(               1  光標回退 N 個句子
)               1  光標前進 N 個句子
*               1  正向搜索第 N 次出現的光標所在的標識符
+               1  等同於 <CR>
...
@@              2  重複上次的 @{a-z} N 次
A               2  在行尾附加文本 N 次
B               1  光標反向移動 N 個 WORD (字串)
["x]C           2  從光標位置到行尾部分，再加上 N-1 行進行修改[到緩衝區 (譯者注: 應為寄存器，下同) x]；等價於 "c$"
["x]D           2  刪除位置到行尾部分，再加上 N-1 行的字符 [到緩衝區 (見上) x]；等價於 "d$"
E               1  光標正向移動到第 N 個 WORD 的結束處
F{char}         1  光標左移到第 N 次出現的 {char} 上
G               1  光標移動到第 N 行，缺省是最後一行
H               1  光標移動到屏幕頂部開始算的第 N 行
I               2  在本行第一個 CHAR 之前插入文本 N 次
J               2  連接 (Join) N 行；缺省為 2
K                  查找光標所在的關鍵字 (Keyword)，使用'keywordprg' 的定義
L               1  光標移動到屏幕底部開始算的第 N 行
M               1  光標移動到屏幕的中間一行
N               1  反方向重複最近的 '/' 或 '?' N 次
O               2  在光標上方開始一個新行，並插入文本，重複 N次
["x]P           2  在光標放置文本 [到緩衝區 (見上) x] N 次
Q                  切換到 "Ex" 模式
R               2  進入替換模式: 覆蓋存在的字符，重複輸入文本N-1 次
["x]S           2  刪除 N 行 [到緩衝區 (見上) x]] 並開始輸入；等價於 "cc"
T{char}         1  光標移動到左側第 N 次出現的 {char} 之前
U               2  撤消一行內所有最近的改變
V                  進入面向行的可視 (Visual) 模式
W               1  光標前移 N 個 WORD
["x]X           2  刪除光標前的 N 個字符 [到緩衝區 (見上) x]
...
do              2  等同於 ":diffget"
dp              2  等同於 ":diffput"
e               1  光標正向移動到第 N 個單詞的結束處
f{char}         1  光標右移到第 N 次出現的 {char} 上
g{char}            擴展命令，見下 |g|
h               1  光標左移 N 個字符
i               2  在光標前插入文本 N 次
j               1  光標下移 N 行
k               1  光標上移 N 行
l               1  光標右移 N 個字符
m{A-Za-z}          在光標位置處設置位置標記 {A-Za-z}
n               1  重複最近的 '/' 或 '?' N 次
o               2  在光標下方開始一個新行，並插入文本，重複 N次
["x]p           2  在光標後放置文本 [從寄存器 x] N 次
...
t{char}         1  光標移動到右側第 N 次出現的 {char} 之前
u               2  撤消改變
v                  開始面向字符的可視 (Visual) 模式
w               1  光標前移 N 個單詞
["x]x           2  刪除光標開始的 N 個字符 [到緩衝區 (見上) x]
...
~               2  'tildeop' 關閉時: 切換光標所在處開始的 N 個字符的大小寫，並右移光標 N 個字符
...
<Insert>        2  等同於 "i"
<Left>          1  等同於 "h"
<LeftMouse>     1  移動光標到鼠標點擊處
<MiddleMouse>   2  等同於在鼠標點擊處 "gP"
<PageDown>         等同於 CTRL-F
<PageUp>           等同於 CTRL-B
<Right>         1  等同於 "l"
<RightMouse>       開始可視模式，移動光標到鼠標點擊處
<S-Down>        1  等同於 CTRL-F
<S-Left>        1  等同於 "b"
...
CTRL-W ]           分割窗口並跳轉到光標所在的標籤上
CTRL-W ^           分割窗口並編輯輪換文件 N
CTRL-W _           設置當前窗口高度為 N (缺省: 儘量大)
CTRL-W b           跳轉到底部的窗口
CTRL-W c           關閉當前窗口 (類似於 |:close|)
CTRL-W d           分割窗口並跳轉到光標所在的定義上
CTRL-W f           分割窗口並跳轉到光標所在的文件名上
CTRL-W F           分割窗口並編輯光標所在的文件名，然後跳轉到文件名後給出的行號上。
CTRL-W g CTRL-]  分割窗口並在光標所在的標籤上執行 |:tjump|
CTRL-W g ]         分割窗口並在光標所在的標籤上執行 |:tselect|
CTRL-W g }         在光標所在的標籤上執行 |:ptjump|
CTRL-W g f         在新的標籤頁裡編輯光標所在的文件名
CTRL-W g F         在新的標籤頁裡編輯光標所在的文件名然後跳轉到文件名後給出的行號上。
CTRL-W h           跳轉到左面第 N 個的窗口 (在首個窗口上停止)
CTRL-W i           分割窗口並跳轉到光標所在的標識符的聲明上
...
CTRL-W z           關閉預覽窗口
CTRL-W |           設置窗口寬度為 N 列
CTRL-W }           在預覽窗口裡顯示光標所在的標籤
CTRL-W <Down>      等同於 "CTRL-W j"
CTRL-W <Up>        等同於 "CTRL-W k"
CTRL-W <Left>      等同於 "CTRL-W h"
CTRL-W <Right>    等同於 "CTRL-W l"
###方括號命令
[ CTRL-D           跳轉到第一個在當前文件和它包含的頭文件裡匹配光標所在的單詞的 #define，從當前文件的頭部開始
[ CTRL-I           跳轉到第一個在當前文件和它包含的頭文件裡匹配光標所在單詞的位置，從當前文件頭部開始
[#              1  光標移動到前面第 N 個未匹配的 #if、#else 或者 #ifdef
['              1  光標移動到前面的小寫位置標記，定位在它所在行的第一個非空白字符上
[(              1  光標移動到前面第 N 個未匹配的 '('
[*              1  等同於 "[/"
[`              1  光標移動到前一個小寫位置標記
[/              1  光標移動到前面第 N 個 C 註釋的開始處
[D                 列出在當前文件和它包含的頭文件裡匹配光標所在的單詞的所有定義，從當前文件的頭部開始
[I                 列出在當前文件和它包含的頭文件裡匹配光標所在單詞的所有位置，從當前文件的頭部開始
[P              2  等同於 "[p"
[[              1  光標後退 N 個小節
[]              1  光標後退 N 個 SECTION
[c              1  光標後退 N 個更改開始的地方
[d                 顯示第一個在當前文件和它包含的頭文件裡匹配光標所在的單詞的 #define，從當前文件的頭部開始
[f                 等同於 "gf"
[i                 顯示第一個在當前文件和它包含的頭文件裡匹配光標所在單詞的位置，從當前文件的頭部開始
[m              1  光標後退 N 個成員函數的開始
[p              2  類似於 "P"，但調整當前行的縮進
[s              1  移動到前一個拼寫錯誤的單詞
[z              1  移動到打開的摺疊的開始
[{              1  光標後退 N 個未匹配的 '{'
[<MiddleMouse>  2  等同於 "[p"
] CTRL-D           跳轉到第一個在當前文件和它包含的頭文件裡匹配光標所在的單詞的 #define，從光標位置開始
] CTRL-I           跳轉到第一個在當前文件和它包含的頭文件裡匹配光標所在單詞的位置，從光標位置開始
]#              1  光標移動到後面第 N 個未匹配的 #endif、#else
]'              1  光標移動到後面的小寫位置標記，定位在它所在行的第一個非空白字符上
](              1  光標移動到後面第 N 個未匹配的 ')'
]*              1  等同於 "]/"
]`              1  光標移動到後一個小寫位置標記
]/              1  光標移動到後面第 N 個 C 註釋的結尾處
]D                 列出在當前文件和它包含的頭文件裡匹配光標所在的單詞的所有定義，從光標位置開始
]I                 列出在當前文件和它包含的頭文件裡匹配光標所在單詞的所有位置，從光標位置開始
]P              2  等同於 "[p"
]]              1  光標前進 N 個小節
]c              1  光標前進 N 個更改開始的地方
]d                 顯示第一個在當前文件和它包含的頭文件裡匹配光標所在的單詞的 #define，從光標位置開始
]f                 等同於 "gf"
]i                 顯示第一個在當前文件和它包含的頭文件裡匹配光標所在單詞的位置，從光標位置開始
]m              1  光標前進 N 個成員函數的結尾
]p              2  類似於 "p"，但調整當前行的縮進
]s              1  移動到後一個拼寫錯誤的單詞
]z              1  移動到打開的摺疊的結尾處
...
g8                 顯示光標所在的 UTF-8 字符的十六進制字節碼
g<                 顯示上次的命令輸出
g?              2  Rot13 編碼操作符
g??             2  Rot13 編碼當前行
g?g?            2  Rot13 編碼當前行
gD              1  轉到當前文件的光標所在單詞的定義
gE              1  反向轉移到上一個 WORD 的結尾處
gH                 啟動選擇行模式
gI              2  類似於 "I"，但總是從第一列開始
gJ              2  連接行，不插入空格
["x]gP          2  在光標籤放置文本 [從寄存器 x] N 次，光標留在插入文本之後
gR              2  進入虛擬替換模式
gU{motion}      2  使得 Nmove 文本全部大寫
gV                 在執行選擇模式下的映射或者菜單時，不要重複選擇過去的可視區域
g]                 :tselect 到光標所在的標籤上
g^              1  'wrap' 關閉時轉到當前行最左側屏幕上可見的非空白字符'wrap' 打開時轉到當前屏幕行最左側的非空白字符
ga                 打印光標所在處的字符的 ascii 值
gd              1  轉到當前函數裡當前光標所在的單詞的定義
ge              1  反向轉到前一個單詞的結尾處
gf                 開始編輯當前光標所在的名字對應的文件
gF                 開始編輯光標所在的文件名並跳轉到文件名後的行號上。
gg              1  光標移動到第 N 行，缺省是第一行
gh                 開始選擇模式
gi              2  類似於 "i"，但先移動到 |'^| 位置標記
gj              1  類似於 "j"，但 'wrap' 打開時往下 N 個屏幕行
gk              1  類似於 "k"，但 'wrap' 打開時往上 N 個屏幕行
gm              1  轉到屏幕行正中間的那個字符
go              1  光標移動到緩衝區的第 N 個字節
["x]gp          2  在光標後面放置文本 [從寄存器 x] N 次，光標留在後面
gq{motion}      2  排版 Nmove 文本
gr{char}        2  虛擬替換 N 個字符為 {char}
gs                 睡眠 N 秒 (缺省 1)
gu{motion}      2  使得 Nmove 文本全部小寫
gv                 重新選擇上次的可視區域
gw{motion}      2  排版 Nmove 文本並保持光標位置
gx                 執行光標下的文件名對應的應用程序 (僅限於|netrw| 插件)
...
g<Up>           1  等同於 "gk"
###'z' 開始的命令
z<CR>              重畫，光標移動到窗口的頂行的第一個非空白字符
z{height}<CR>      重畫，使得窗口高度為 {height} 行
z+                 光標移動到第 N 行 (缺省為窗口之後的第一行)，其它同 "z<CR>"
z-                 重畫，光標移動到窗口末行的第一個非空白字符
z.                 重畫，光標移動到窗口的中間行的第一個非空白字符
z=                 給出拼寫建議
...
zW                 把單詞標記為拼寫錯誤 (wrong)
zX                 重新應用 'foldlevel'
z^                 光標移動到第 N 行 (缺省為窗口之前的一行)，其它同 "z-"
za                 打開關閉的摺疊，或關閉打開的摺疊
zb                 重畫，光標在窗口底行
zc                 關閉摺疊
zd                 刪除摺疊
ze                 'wrap' 關閉時，水平滾動使得光標定位到屏幕的尾部 (最右側)
...
zo                 打開摺疊
zr                 給 'foldlevel' 加一
zs                 'wrap' 關閉時，水平滾動使得光標定位到屏幕的起始處 (最左側)
zt                 重畫，光標移動到窗口的頂部
zw                 把單詞標記為拼寫錯誤 (wrong)
zv                 打開足夠的摺疊，使得當前行可見
...
###命令行編輯
CTRL-@          不用
CTRL-A          使用光標前面的模式進行補全，並插入所有的匹配
CTRL-B          光標移動到命令行開始的地方
CTRL-C          等同於 <ESC>
CTRL-D          列出光標前面模式的所有補全
CTRL-E          光標移動到命令行結束的地方
CTRL-F          'cedit' 的缺省值: 打開命令行窗口；否則不用
CTRL-G          不用
<BS>            刪除光標前面的字符
{char1} <BS> {char2}   'digraph' 打開時，輸入二合字母
CTRL-H          等同於 <BS>
<Tab>           如果 'wildchar' 是 <Tab>: 補全光標前面的模式
<S-Tab>         等同於 CTRL-P
'wildchar'      補全光標前面的模式 (缺省: <Tab>)
CTRL-I          等同於 <Tab>
<NL>            等同於 <CR>
CTRL-J          等同於 <CR>
CTRL-K {char1} {char2}輸入二合字母
CTRL-L          補全光標前的模式，並插入最長的公共子串
<CR>            執行輸入的命令
CTRL-M          等同於 <CR>
...
CTRL-R {0-9a-z"%#*:= CTRL-F CTRL-P CTRL-W CTRL-A}插入寄存器的內容或特殊對象到光標所在的地方，如同直接輸入那樣
CTRL-R CTRL-R {0-9a-z"%#*:= CTRL-F CTRL-P CTRL-W CTRL-A}按本義插入寄存器的內容或特殊對象到光標所在的地方
CTRL-S          (用於終端控制流)
CTRL-T          不用
CTRL-U          刪除所有字符
...
<S-Right>       光標右移一個單詞
<C-Right>       光標右移一個單詞
<Up>            從歷史中調入匹配光標前模式的前一個命令行
<S-Up>          從歷史中調入前一個命令行
<Down>          從歷史中調入匹配光標前模式的後一個命令行
<S-Down>        從歷史中調入後一個命令行
<Home>          光標移動到命令行開始的地方
<End>           光標移動到命令行結束的地方
<PageDown>      等同於 <S-Down>
