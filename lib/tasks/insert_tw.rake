...
###方括號指令
[ CTRL-D           跳轉到第一個在目前檔案和它包含的標頭檔裡匹配游標所在的單字的 #define，從目前檔案的頭部開始
[ CTRL-I           跳轉到第一個在目前檔案和它包含的標頭檔裡匹配游標所在單字的位置，從目前檔案頭部開始
...
[D                 列出在目前檔案和它包含的標頭檔裡匹配游標所在的單字的所有定義，從目前檔案的頭部開始
[I                 列出在目前檔案和它包含的標頭檔裡匹配游標所在單字的所有位置，從目前檔案的頭部開始
[P              2  等同於 "[p"
[[              1  游標後退 N 個小節
[]              1  游標後退 N 個 SECTION
[c              1  游標後退 N 個更改開始的地方
[d                 顯示第一個在目前檔案和它包含的標頭檔裡匹配游標所在的單字的 #define，從目前檔案的頭部開始
[f                 等同於 "gf"
[i                 顯示第一個在目前檔案和它包含的標頭檔裡匹配游標所在單字的位置，從目前檔案的頭部開始
[m              1  游標後退 N 個成員函數的開始
[p              2  類似於 "P"，但調整該行的縮排
[s              1  移動到前一個拼寫錯誤的單字
[z              1  移動到開啟的摺疊的開始
[{              1  游標後退 N 個未匹配的 '{'
[<MiddleMouse>  2  等同於 "[p"
] CTRL-D           跳轉到第一個在目前檔案和它包含的標頭檔裡匹配游標所在的單字的 #define，從游標位置開始
] CTRL-I           跳轉到第一個在目前檔案和它包含的標頭檔裡匹配游標所在單字的位置，從游標位置開始
...
]D                 列出在目前檔案和它包含的標頭檔裡匹配游標所在的單字的所有定義，從游標位置開始
]I                 列出在目前檔案和它包含的標頭檔裡匹配游標所在單字的所有位置，從游標位置開始
]P              2  等同於 "[p"
]]              1  游標前進 N 個小節
]c              1  游標前進 N 個更改開始的地方
]d                 顯示第一個在目前檔案和它包含的標頭檔裡匹配游標所在的單字的 #define，從游標位置開始
]f                 等同於 "gf"
]i                 顯示第一個在目前檔案和它包含的標頭檔裡匹配游標所在單字的位置，從游標位置開始
]m              1  游標前進 N 個成員函數的結尾
]p              2  類似於 "p"，但調整該行的縮排
]s              1  移動到後一個拼寫錯誤的單字
]z              1  移動到開啟的摺疊的結尾處
...
<S-Right>       游標右移一個單字
<C-Right>       游標右移一個單字
<Up>            從歷史中調入匹配游標前模式的前一個指令列
<S-Up>          從歷史中調入前一個指令列
<Down>          從歷史中調入匹配游標前模式的後一個指令列
<S-Down>        從歷史中調入後一個指令列
<Home>          游標移動到指令列開始的地方
<End>           游標移動到指令列結束的地方
<PageDown>      等同於 <S-Down>
