...
Be Vimmer 是一個可定期在 tweeter 上發佈 Vim 指令及相關敘述 (日文、英文及簡繁體中文) 的應用程式，以於幫助 Vim 初學者。
...
推特文將會在 server 上以 cron 任務的方式進行發佈。 (詳見 +lib/tasks/cron.rb+) 在執行此應用程式前，使用者需先將註冊的推特資訊設定在環境變數中。
...
下列指令說明：每隔兩小時以日文發佈三則推特文。EX 指令每六小時出現一次。
